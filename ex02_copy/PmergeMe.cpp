...
/*   Updated: 2023/09/20 23:19:55 by elsie            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (solo == -1) {
		while (_main_chain.size() != list.size() && i < _main_chain.size()) {
			...
			if (jacobsthalNb(js_index) >= _main_chain.size()) {
				...
				while (start < _main_chain.size()) {
					if (_main_chain[start][1] == 0) {
						nbToInsert = findFirstNbPair(pairs, _main_chain[start][0], start);
						insertNbInMainChain(nbToInsert, start);
				...
				while (i < jacobsthalNb(js_index))
					i++;
				nbToInsert = findFirstNbPair(pairs, _main_chain[i][0], i); // <=> _main_chain[jacobsthalNb(js_index)]
				...
				j = i;
				while (j < save) {
					nbToInsert = findFirstNbPair(pairs, _main_chain[j][0], j);
					//std::cout << BLUE << "Nbtoinsert: " << nbToInsert << DEFAULT << std::endl;
					if (nbToInsert != -1)
